...
// ─────────────────────────────────────────────────────────────────────────────
// Business-specific context for Cofiesto branches in Baku:
...
export const BAKU_EVENTS = [
  ...
  { name: "New Year holidays", start: "2025-12-31", end: "2026-01-02", window: true,
    type: "public_holiday",
    impact: "Offices closed (City Point low); festive, tourist & leisure footfall up in the centre (Zarifa, Central Park)." },
  ...
  { name: "Novruz holiday", start: "2026-03-20", end: "2026-03-30", window: true,
    type: "public_holiday",
    impact: "Long break: offices closed (City Point very low) and many residents leave Baku, softening local footfall; tourist traffic moderate." },
  { name: "Victory & Peace Day", start: "2026-05-09", end: "2026-05-09", window: true,
    type: "public_holiday", impact: "Office sites (City Point) lower; central leisure footfall normal-to-up." },
  { name: "Qurban Bayramı + Independence Day", start: "2026-05-27", end: "2026-05-28", window: true,
    type: "public_holiday", impact: "Offices closed (City Point low); some residents travel; central/tourist sites steadier." },
  ...
  { name: "Women's European Volleyball Championship (Baku co-hosts)", start: "2026-08-21", end: "2026-09-06", window: false,
    type: "sports_event",
    impact: "International visitor uptick in late summer — upside for the tourist/central branches (Zarifa especially)." },
  { name: "Formula 1 Azerbaijan Grand Prix", start: "2026-09-23", end: "2026-09-26", window: false,
    type: "mega_event",
    impact: "The single biggest tourist surge of the year. The street circuit runs through the city centre / Old City right next to Zarifa (Port); expect a large, high-spend influx around the race and the Crystal Hall concerts (Katy Perry, Calvin Harris headline 2026). Plan staffing, stock and hours for Zarifa and central sites; book this window every year (contract runs to 2030)." },
];
...
export function buildBusinessContextBlock(weatherNote) {
  ...
  lines.push("");
  lines.push("=== AZERBAIJAN 2026 WORKING-DAY CALENDAR (key for City Point) ===");
  lines.push("Compare months by revenue PER WORKING DAY for office-dependent City Point. Working days per month:");
  ...
  if (weatherNote) {
    lines.push("");
    lines.push("=== CURRENT BAKU WEATHER (relevant to Central Park footfall) ===");
...
export function buildContextInsights(dashboardData, lang) {
  ...
  if (next) {
    ...
    out.push({
      ...
      az: `Yaxınlaşan tədbir: ${next.name} (${next.start})${days?` — ${days} gün qalıb`:""}. Mərkəzi və turist filialları (xüsusən Zarifa) üçün hazırlaş: kadr, ehtiyat, iş saatları.`,
      ru: `Ближайшее событие: ${next.name} (${next.start})${days?` — через ${days} дн.`:""}. Подготовьте центральные/туристические филиалы (особенно Zarifa): персонал, запасы, часы работы.`,
      en: `Upcoming event: ${next.name} (${next.start})${days?` — in ${days} days`:""}. Prepare the central/tourist branches (Zarifa especially): staffing, stock, opening hours.`,
    });
  }

  // (b) City Point — revenue per working day (calendar-fair view)
  const cp = dashboardData?.pos?.["City Point"];
  if (cp?.monthly?.length >= 2) {
    ...
    if (withWD.length >= 2) {
      ...
      out.push({
        id: "citypoint_workday", tone: "slate",
        az: `City Point ofis mərkəzindədir — ədalətli müqayisə iş günü başına gəlirdir. Son ay ${b.wd} iş günü (əvvəlki ${a.wd}). Xam dəyişim ${rawMoM>=0?"+":""}${rawMoM.toFixed(1)}%, amma iş günü başına ${perMoM>=0?"+":""}${perMoM.toFixed(1)}% — fərq təqvimdəndir, tələb itkisi deyil.`,
        ru: `City Point — в бизнес-центре; честнее сравнивать выручку на рабочий день. В последнем месяце ${b.wd} раб. дн. (ранее ${a.wd}). Грубое изменение ${rawMoM>=0?"+":""}${rawMoM.toFixed(1)}%, но на рабочий день ${perMoM>=0?"+":""}${perMoM.toFixed(1)}% — разница из-за календаря, а не падения спроса.`,
        en: `City Point sits in an office centre — the fair comparison is revenue per working day. Latest month had ${b.wd} working days (prev ${a.wd}). Raw change ${rawMoM>=0?"+":""}${rawMoM.toFixed(1)}%, but per working day ${perMoM>=0?"+":""}${perMoM.toFixed(1)}% — the gap is calendar, not lost demand.`,
      });
    }
  }

  // (c) Central Park — weather/season note
  out.push({
    id: "central_weather", tone: "green",
    az: "Central Park parkın yanındadır və havadan asılıdır — mülayim aylar (apr–iyun, sen–okt) və günəşli həftəsonları ən güclü vaxtlardır; istiyə/yağışa görə düşüş normaldır.",
    ru: "Central Park у парка и зависит от погоды — мягкие месяцы (апр–июнь, сен–окт) и солнечные выходные сильнее всего; спад в жару/дождь — норма.",
    en: "Central Park is park-side and weather-driven — mild months (Apr–Jun, Sep–Oct) and sunny weekends are strongest; dips in heat/rain are normal.",
  });
